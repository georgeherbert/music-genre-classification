...
\]
where $x$ is the output from the previous layer; $\hat{x}$ is the input to the subsequent layer; and $i$, $c$, $x$ and $y$ are the batch, channel, width and height indices;

Batch normalisation has strong empirical performance, but there is some disagreement as to the theory behind its effectiveness.
