...
The training curves in Figure \ref{accuracy_curves} and Figure \ref{loss_curves} highlighted that the network was severely overfitting.
To improve the performance of my network, I experimented with data augmentation: a common technique employed to reduce overfitting that involves the creation of new synthetic data.
In the context of audio data, augmentation can take several forms; Schindler et al. experimented with two forms of augmentation: pitch shifting and time stretching.
I opted to implement pitch shifting: a technique in which the original pitch of an audio sampled is raised or lowered, whilst the tempo is unchanged.
I implemented the same shifts as Schindler et al.: raising and lowering the pitch by 2 and 5 semitones.
I applied each of these four shifts to three samples randomly selected from each audio file.

Unlike Schindler et al., training my network on the augmented dataset yielded a small improvement in accuracy.
Furthermore, the training curves confirmed that the degree to which my network was overfitting was significantly reduced.
